...
const AdminAddProduct = () => {
  const [isSuccess , isError , error , images , setImages , crop , title_input , description_input , price_after_input , price_input , quantity_input,optionsCategory,catinput,hundleonChangeCategory,animatedComponents,hundleonChangeSubCategories,subcatinput,optionsSubCategory,brandinput,hundleOnChangeBrand,optionsbrand,colors,hundleOnClickDeleteColor,hundleOnClickColor,hundleOnChangeColor,hundleClickAdd,isLoading,newColor] = AddProductHook();
  return (
    <div  className='position-relative'>
      <div style={{direction:"rtl"}}>
        {isSuccess ? <SuccessMessage message={"Product Created Successfully"}/>:""}
    ...
    <Form.Group className="mb-3">
                <Form.Control ref={title_input} type="text" placeholder="product title" />
            </Form.Group>
            <Form.Group className="mb-3" controlId="exampleForm.ControlTextarea1">
                <Form.Control ref={description_input} as="textarea" placeholder='product description' rows={3} />
            </Form.Group>
            <Form.Group className="mb-3">
                <Form.Control ref={price_after_input} type="text" placeholder="product price after discount" />
            </Form.Group>
            <Form.Group className="mb-3">
                <Form.Control ref={price_input} type="text" placeholder="product price" />
            </Form.Group>
            <Form.Group className="mb-3">
        ...
        />  
            <div class="gap-2 mt-3 d-flex" role="group" aria-label="Basic group">
              {colors.map((item , index) => {
                ...
                return <label value='click to delete' onClick={()=>hundleOnClickDeleteColor(index)} class="color-item rounded-circle" style={{backgroundColor:`${item}`}} for="btnradio4"></label>
              })}
    ...
    <Button className='mt-3 w-100  fw-semibold' variant='success' onClick={hundleClickAdd}>ADD THE PRODUCT
