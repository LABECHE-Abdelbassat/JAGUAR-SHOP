...
import { useCreateCouponMutation, useDeleteCouponMutation, useGetAllCouponsQuery } from '../../reduxQuery/APIs/couponApi'
...
const AdminCoupon = () => {
    ...
    const [deleteCoupon , {isSuccess:deleteSuccess , isError:deleteIsError , error:deleteError}] = useDeleteCouponMutation();
  ...
  return (
    <div className='position-relative'>
        <div style={{direction:"rtl"}}>
            {isSuccess ? <SuccessMessage message={"Coupon Created Successfully"}/>:""}
            {isError ? <ErrorMessage error={error}/> : ""}
        </div>
        <div style={{direction:"rtl"}}>
            {deleteSuccess ? <SuccessMessage message={"Coupon Deleted Successfully"}/>:""}
        ...
        <Form>
            <Form.Group className='mt-3' as={Col} controlId="formGridPassword">
                <Form.Control ref={coupon_name_input} type="text" placeholder="Coupon name" />
            </Form.Group>
            <Form.Group className='mt-3' as={Col} controlId="formGridPassword">
                <Form.Control ref={coupon_end_data_input} type="date" placeholder="06/11/2024" />
            </Form.Group>
            <Form.Group className='mt-3 mb-3' as={Col} controlId="formGridPassword">
                <Form.Control ref={coupon_discount_input} type="number" placeholder="% discount" />
            </Form.Group>
            <Button onClick={hundleClickAddCoupon} variant="success" className='w-100' type="submit">
                Add Coupon
                    ...
                    {data?.data.map((item, index)=>{
                        return <Card key={index} className='mt-3'>
                            <Card.Body className='d-flex justify-content-between'>
                                <div><strong>Coupon Name : </strong>{item.name}</div>
                                <CloseButton onClick={()=>hundleClickDeleteCoupon(item._id)}></CloseButton>
                            </Card.Body>
                            <Card.Body><strong>Expire Date : </strong>{item.expire.slice(0,10)}</Card.Body>
                            <Card.Body><strong>Discount : </strong>{item.discount}</Card.Body>
                        </Card>
                    })}
                </div>
            }
